...
int main()
{
    ...
    // --------------------
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Coordenadas OpenGL : Quadrado", NULL, NULL);
    if (window == NULL)
    {
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        float giro = (float) glfwGetTime() * velocidade;

        // create transformations
        glm::mat4 model         = glm::mat4(1.0f); // make sure to initialize matrix to identity matrix first
        glm::mat4 view          = glm::mat4(1.0f);
        glm::mat4 projection    = glm::mat4(1.0f);
        model = glm::rotate(model, glm::radians(-55.0f + giro), glm::vec3(0.0f, 0.0f, -1.0f));
